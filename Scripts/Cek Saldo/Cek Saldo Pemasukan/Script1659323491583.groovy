...
nominal = WebUI.callTestCase(findTestCase('Pemasukan/Hari Ini/Pemasukan Hari Ini - Deposito'), [:], FailureHandling.STOP_ON_FAILURE)
...
newPemasukan1 = newPemasukan.replace('.', '')

println(newPemasukan1)

newPengeluaran = newPengeluaran.replace('.', '')

newPengeluaran = newPengeluaran.toInteger()

println(newPengeluaran)

newSaldo = newSaldo.replace('.', '')

newSaldo = newSaldo.toInteger()

println(saldo)

Mobile.verifyMatch(saldoAkhir, newPemasukan1, false)
